  ...
  export default {
    ...
    methods: {
      _setChart(chartConfig, initChart = false, hasData = false) {
        ...
        //   myChart.resize!==null && myChart.resize()
        // })
        return myChart
      },
      _setOption(chartConfig) {
            ...
            formatter: function(params) {
              ...
              params.forEach(function(item) {
                ...
                result += `<p><span style="display:inline-block;margin-right:5px;margin-bottom:-1px;width:10px;height:10px;border-radius:1px;background-color:${
                  item.color
                }"></span><span style="color:#ffffff;font-size:12px">${item.seriesName}: ${
                  item.value
                }${hasUnit}</span></p>`
              })
              return result
        ...
        for (let i = 0; i < chartConfig.dataArr.length; i++) {
          ...
          series.push(this._setSeries(chartConfig, item, i)) // 设图表类型样式,及数据
        ...
        option.yAxis = this._setYAxis(chartConfig.showSecondY) // 不显示Y轴
        return option
      },
      _setSeries(chartConfig, item, i) {
        ...
        if (chartConfig.lineShadow !== 'false' && (chartConfig.dataArr.length <= 2 || chartConfig.lineShadow)) {
          let color = [
            'rgba(96,129,227,0.28)',
            'rgba(136,89,232,0.28)',
            'rgba(247,193,54,0.28)',
            'rgba(106,225,255,0.28)'
          ]
