...
export const actions = {
  reqGroupList({commit, state, dispacth}, {parentId, loading = true}) {
    return API.OutreachGroup.getGroupList({parent_id: parentId}, loading).then((res) => {
  ...
  addDepartment({commit, state, rootState}, args) {
    ...
    const data = {
      parent_id: state.addPosition.current.id,
  ...
  deleteDepartment({commit, state}, args) {
    ...
    return API.OutreachGroup.deleteDepartment({id: current.id}, true).then((res) => {
      if (res.error !== app.$ERR_OK) {
        app.$toast.show(res.message)
        return
      }
  ...
  editDepartment({commit, state, rootState}, args) {
    ...
    return API.OutreachGroup.editDepartment(data, true).then((res) => {
      if (res.error !== app.$ERR_OK) {
        app.$toast.show(res.message)
        return
      }
  ...
  moveDepartment({commit, state}, args) {
    ...
    return API.OutreachGroup.moveDepartment({list}, true).then((res) => {
      if (res.error !== app.$ERR_OK) {
        app.$toast.show(res.message)
        return
      }
